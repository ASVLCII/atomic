...
npm run build
npm run test:integration -- test/integration/postgres-real-isolation.test.ts test/integration/postgres-real-dependency-failure.test.ts test/integration/workflow-postgres-real-outage.test.ts
```
...
| Database lost before admission | Actual rejected admission reaches `blocked_dependency`, with responsive public status and no author work; pause acknowledges promptly and resumes the same ID after recovery; quit acknowledges without late author work |
...
recovery cannot erase the outage before the control assertion. Managed automatic recovery
is exercised separately by the multi-process isolation tests. A callback interrupted
