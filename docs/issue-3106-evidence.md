...
- S1–S8: provider state tests preserve raw attributed text, whitespace, duplicates/order, empty or omitted arrays, unavailable sources, labeled summaries and explicit quoted constraints. A path can remain a task target. Path-only document evidence is rejected before inference; the router does not dereference it. Existing credential suites verify pre-inference filtering. These tests verify content transport and constraints, not probabilistic model obedience.
