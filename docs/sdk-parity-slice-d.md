...
| --- | --- | --- |
| Human capability independent of rendering | Node callback workflow launch remains interactive without terminal UI | Actual factory/discovery/extension launch and primitive input/confirm pass |
...
| Durable missing adapter | Separately reached primitive confirm remains pending, never skipped | Pass: initial missing confirm and restored confirm before host binding |
...
Final commands on the candidate:

- `npm run build` and `npm run check`: exit 0.
- `npx vitest --run --project integration test/integration/sdk-builtin-host-parity.test.ts test/integration/workflow-prompt-tool-resume.test.ts test/integration/postgres-managed-dbos-recovery.test.ts`: 29 tests, 3 files, no skips; host parity contributes 25 tests.
...
- Focused unit command recorded in the final progress receipt: 273 tests, 19 files, including broker/prompt/readiness/rebind/reload, persistence, budget and documentation architecture.
...
- `node test/fixtures/sdk-host-terminal.mjs` in dedicated tmux: `terminal-proof`, approval true, refusal false, exit 0. Text captures and raw terminal output retained with the task evidence.
