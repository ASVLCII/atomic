  ...
  quote! {
    ...
    impl napi::bindgen_prelude::FromNapiRef for #name {
      unsafe fn from_napi_ref(
        ...
        let wrapped_val = std::ptr::NonNull::new(wrapped_val.cast::<#name>()).ok_or_else(|| {
          napi::Error::new(
            napi::Status::InvalidArg,
        ...
        // Reject a wrong-class / prototype-spoofed `&T` argument before
        // constructing a reference from the checked pointer. `validate_type_tag`
    ...
    impl napi::bindgen_prelude::FromNapiMutRef for #name {
      unsafe fn from_napi_mut_ref(
        ...
        let mut wrapped_val = std::ptr::NonNull::new(wrapped_val.cast::<#name>()).ok_or_else(|| {
          napi::Error::new(
            napi::Status::InvalidArg,
            "Cannot borrow a null native value".to_owned(),
          )
        ...
        // Reject a wrong-class / prototype-spoofed `&mut T` argument before
        // constructing a reference from the checked pointer. `validate_type_tag`
        // `napi8` feature (the gate lives inside napi, since this code expands
        // in the consumer crate where `cfg(feature = "napi8")` would not see
        // napi's features).
        ...
        getters_setters.push((
          ...
          quote! {
            unsafe fn #getter_name(
              ...
              #tracing_debug
              let mut this_ptr = std::ptr::NonNull::new(unsafe {
              ...
              let _napi_native_borrow =
                napi::bindgen_prelude::acquire_native_borrow(this_ptr.as_ptr(), true)?;
              let obj: &mut #struct_name = unsafe { this_ptr.as_mut() };
