      ...
      match self.fn_self {
        Some(FnSelf::Ref) => {
          ...
          receiver_unwrap = quote! {
            let this_ptr = std::ptr::NonNull::new(cb.unwrap_raw::<#parent>()?).ok_or_else(|| {
              napi::Error::new(
          ...
          receiver_conversion = quote! {
            let this: &#parent = this_ptr.as_ref();
          };
        }
        Some(FnSelf::MutRef) => {
          refs.push(make_ref(quote! { cb.this() }));
          receiver_unwrap = quote! {
            let mut this_ptr = std::ptr::NonNull::new(cb.unwrap_raw::<#parent>()?).ok_or_else(|| {
              napi::Error::new(
                napi::Status::InvalidArg,
                "Cannot borrow a null native value".to_owned(),
              )
          ...
          receiver_conversion = quote! {
            let this: &mut #parent = this_ptr.as_mut();
          };
        }
        _ => {}
      };
