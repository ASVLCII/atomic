...
Sources: [index](https://artificialanalysis.ai/evaluations/artificial-analysis-intelligence-index); [methodology](https://artificialanalysis.ai/methodology/intelligence-benchmarking). Constituents include [AA-Briefcase v1.1](https://artificialanalysis.ai/evaluations/aa-briefcase), [GDPval-AA v2.1](https://artificialanalysis.ai/evaluations/gdpval-aa), [Humanity's Last Exam](https://artificialanalysis.ai/evaluations/humanitys-last-exam), [AA-LCR v1.1](https://artificialanalysis.ai/evaluations/artificial-analysis-long-context-reasoning), AutomationBench-AA, Terminal-Bench 4.0, SciCode, GDP.pdf, CritPt and Omniscience; GPQA/MMMU-Pro are not constituents.

47-config default-chart union: 31 aggregate rows plus default constituent selections (311 displayed constituent records; CritPt has 32). Columns: `idx`=AA Index points; `Bn/Gn`=normalized Elo points; `Auto/TB/Sci/HLE/PDF/Crit/OA/ONH/LCR`=%; `OA`=Omniscience accuracy and `ONH`=non-hallucination. Raw Briefcase/GDPval Elo and the separate −100…100 Omniscience headline are omitted here because their normalized/component values are retained. Normalized Elo is `clamp((Elo−500)/2000)*100`; PDF is GDP.pdf All-pass; Omniscience 6,000-question ONH is `1−hallucination=(partial+notattempted)/(incorrect+partial+notattempted)`. Fable rows use Adaptive Reasoning; Fable 5 fallback=Opus 4.8, Fable 5.1 Default Fallback.

AA row identities use source `chart_label`, except `full_config_label` where the chart omits material settings; no effort is inferred from slug. Inkling AA `xhigh` is distinct from Frontier `0.99`.
...
Source: https://deepswe.datacurve.ai/; changelog https://deepswe.datacurve.ai/changelog. Update 2026-09-03. 113 tasks/91 repos/5 languages; mini-swe-agent. All 21 default Best rows in source order, each with source `n_runs=4`. Columns: model, effort, pass@1±95% run-to-run CI (percent), USD/task, output k-tokens, steps. CI=`1.96*std(runs)/sqrt(R)`. Source costs are not billed Atomic cost: GPT-6 Astra uses expected launch pricing, DeepSeek peak rates (off-peak half), GPT-5.6 Sol promotional pricing.
...
Sources: https://cognition.com/frontiercode; data https://cognition.com/data/frontiercode-leaderboard/data.json; methods https://cognition.com/blog/frontier-code-1.1 and https://cognition.com/blog/frontier-code. Revision 2026-07-07; v1.1, Main=100 tasks, Extended=150, Best rows in source order. Main columns: model, source effort label, harness, score, pass, flag (percent), USD/task, output k-tokens. Score is a weighted aggregate; blocking/unfair-use failures are zeroed. Extended values remain linked (not routed). Harness: `cc`=claude-code, `gb`=grok-build, `msa`=mini-swe-agent. `—`=source null, not 0; Inkling `0.99` is unexplained.
