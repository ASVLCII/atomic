...
With **Automatic**, a Jev HTTP or connection failure triggers one attempt with your current chat model, using the same routing context and remaining deadline. Atomic reports the fallback; it can incur chat-provider charges. An explicit router selection disables this behavior. Cancellation, timeout, invalid answers and local validation failures do not trigger fallback.
