...
When `routerModel` is empty and automatically selected Jev returns an HTTP or connection error, routing makes one fallback attempt with the current chat model. A warning reports the safe Jev error and fallback model. The fallback uses the original state, schema and constraints within the remaining deadline, with no further repairs. The result includes `fallback: { from, to, reason }` and identifies the chat model in `model`. This can send routing context to your chat provider and incur its normal charges. Set an explicit `routerModel` to pin routing to one provider and disable this fallback. Missing credentials, invalid answers, response-size violations, cancellation and timeout do not trigger fallback.
