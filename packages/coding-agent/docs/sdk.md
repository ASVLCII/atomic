...
Disposal also waits for already-admitted prompt/compaction hooks, compaction providers and reload preparation. Ensure your extension and `beforeSessionStart` callbacks settle; a pending callback is not completed cleanup. A callback released after shutdown begins cannot start a provider turn, append compaction results or publish a reload candidate. Independent sessions may borrow the same `DefaultResourceLoader`, settings or `eventBus` without sharing workflow ownership; closing one session leaves its sibling's pending workflows alone.
