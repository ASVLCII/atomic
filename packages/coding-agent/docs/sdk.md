...
Retained cleanup is not retained authority: after self-reload returns, neither the invoking command nor another holder may use its captured old `pi` or `ctx` to mutate the successor or obtain a task host. Use capabilities delivered to the new generation. The old shutdown handler still receives the capabilities needed for cleanup when its admitted continuation settles.
