...
`route` returns `workflowType`, a code-generated `workflowId`, `routerDecision`, and the selected definition's actual `inputSchema`, including defaults. It does not launch. `none` returns an empty ID and reserves nothing. Read the estimate from `routerDecision.estimatedDuration`; route and registered run results have no top-level `estimatedDuration`. The nested decision also contains the selection and preserved `maxBudget` override declaration; omitted budget fields inherit their limits at execution.
