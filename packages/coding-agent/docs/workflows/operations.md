...
Malformed or schema-invalid routing answers receive up to three corrective retries after the initial attempt, independently for Jev and any current-chat fallback, within one shared 30-second deadline. With an empty `routerModel`, oversized Jev context, exhausted Jev repairs or a Jev HTTP/connection error triggers a reported switch to the current chat model. Explicit router selections remain pinned. An empty eligible catalog, stale decisions, credential-bearing routing context, timeout and cancellation prevent stage execution without fallback. Choose router and chat providers permitted to receive the stage task and reference context.
...
The router has one 30-second deadline. By default, each provider gets an initial attempt plus three corrective retries for malformed or schema-invalid answers. A valid decision stops repairs; a valid `none` is never rerouted. With an empty `routerModel`, oversized Jev context, a Jev HTTP or connection failure, or exhausted output repairs triggers a reported switch to the current chat model, which gets its own repair allowance within the remaining deadline. Explicit router selections remain pinned. Invalid inputs, cancellation, timeout and stale-catalog rejection do not trigger fallback. Neither repairs nor fallback can launch a duplicate workflow.

Jev asks independent questions about workflow fit, authorization, lifecycle benefit and duration. Atomic preserves exact user budgets in code rather than asking Jev to choose numbers. Workflow contracts appear only with their candidate, not as a second complete catalog repeated in every batch.
