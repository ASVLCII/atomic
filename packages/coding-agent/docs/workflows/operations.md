...
Malformed or schema-invalid routing answers receive up to three corrective retries after the initial attempt, independently for Jev and any current-chat fallback. Model selection has no built-in wall-clock deadline; cancellation and independent provider limits still apply. With an empty `routerModel`, oversized Jev context, exhausted Jev repairs or a Jev HTTP/connection error triggers a reported switch to the current chat model. Explicit router selections remain pinned. An empty eligible catalog, stale decisions, credential-bearing routing context and cancellation prevent stage execution without fallback. Choose router and chat providers permitted to receive the stage task and reference context.
