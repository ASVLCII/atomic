...
Run-level `quit` is different from interrupting the startup request: it preserves a paused run rather than cancelling its durable record. During database admission, workflow execution stops immediately; durable acknowledgement waits for registration within the existing 10-second admission deadline. If registration fails or times out, quit reports the persistence error and leaves the local run paused; this is not confirmation that the database recorded the pause. A continuation with saved durable progress remains resumable. Without usable durable progress, even a successful quit leaves the run nonresumable. Restore database availability and inspect status before retrying or starting a new run.
...
Aborting a call is the point of no return: that callback's executor is already committed to suspending. If the durable paused transition then fails or is refused, Atomic still records the pause locally rather than reporting a stopped run as running. Existing usable durable progress stays resumable; the failed transition does not create progress for a run that has none. The reported error distinguishes the local stop from a confirmed database pause. After successful database admission, running `/workflow quit` again re-attempts a failed paused transition. If admission itself failed, repeated quit instead reports that original admission error without waiting on its abandoned database operation; it does not repair an incomplete durable identity. Retain the run ID and follow the database recovery guidance below.
...
If `pause` during database admission reports a timeout or unavailable database, use `/workflow resume <run-id>` to release the paused attempt. This first resume reports the original admission error and lets the run finish as failed; it does not restart workflow execution. Inspect `/workflow status <run-id>`, restore PostgreSQL, then retry resume with the same full run ID in the current session. Do not start a replacement run or delete database records to bypass the error.
...
Cancellation or the deadline can leave a stored run identity without valid admission metadata, even when PostgreSQL is healthy but slow. No workflow author code ran for that attempt, but another session cannot resume the incomplete record and may report `Workflow <id> has no valid current DBOS state`. Automatic reconciliation is not available. Keep the full run ID and report the failure rather than deleting or resetting database records to force a retry.
