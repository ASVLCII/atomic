...
Malformed or schema-invalid routing answers receive up to three corrective retries after the initial attempt, independently for Jev and any current-chat fallback, within one shared 30-second deadline. With an empty `routerModel`, exhausted Jev repairs or a Jev HTTP/connection error triggers a reported switch to the current chat model. Explicit router selections remain pinned. Missing guides, an empty eligible catalog, stale decisions, credential-bearing routing context, timeout and cancellation prevent stage execution without fallback. Choose router and chat providers permitted to receive the stage task and reference context.
...
The router has one 30-second deadline. By default, each provider gets an initial attempt plus three corrective retries for malformed or schema-invalid answers. A valid decision stops repairs; a valid `none` is never rerouted. With an empty `routerModel`, a Jev HTTP or connection failure, or exhausted output repairs, triggers a reported switch to the current chat model, which gets its own repair allowance within the remaining deadline. Explicit router selections remain pinned. Invalid inputs, cancellation, timeout and stale-catalog rejection do not trigger fallback. Neither repairs nor fallback can launch a duplicate workflow.

Jev packs workflow and budget Choice questions together where context permits. It preserves exact budget values rather than generating or rounding numbers.

Catalogs above 255 choices use tournament requests. Every workflow participates, three candidates survive each batch, and finalists receive a shared comparison. `none` remains available in the final comparison even if eliminated earlier, so overflow never forces a launch.
