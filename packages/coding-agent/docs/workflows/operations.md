...
The router has one 30-second deadline for the initial attempt and up to three repair retries for malformed or schema-invalid answers. A valid decision stops repairs; a valid `none` is never rerouted. Authentication/provider failures, invalid inputs, cancellation, timeout, and stale-catalog rejection are not retried. Repairs use the same provider and context, without an agent loop, provider fallback, or duplicate launch.
...
A replaced or closed host invalidates post-mortem chat handles. Nodes without a valid retained conversation remain read-only: prompts, boundaries, skipped nodes without a conversation, live stages owned elsewhere, and missing or invalid session files.
