...
For an admitted live executor with no active stage or tool, pause installs a local barrier immediately and waits at most 500ms for database confirmation. The acknowledgement and status distinguish `observed` from `durable` control persistence. `observed` is not proof of a saved pause and does not guarantee cross-process resume. During registration, pause acknowledges immediately while database settlement continues in the background. After registration, resume requires database confirmation before releasing the barrier; a failed resume leaves it paused. These limits do not cover stage cancellation or arbitrary external I/O.
