...
Aborting a call is the point of no return: that callback's executor is already committed to suspending. So if the durable paused transition then fails or is refused, Atomic still records the pause locally — the run is never left reported as running with nothing running it — but does not advertise it as resumable, and the reported error names both the durable failure and what it left behind. After successful database admission, running `/workflow quit` again re-attempts a failed paused transition and upgrades the run to resumable once it lands. If admission itself failed, repeated quit instead reports that original admission error without waiting on its abandoned database operation; it does not repair an incomplete durable identity. Retain the run ID and follow the database recovery guidance below.
