...
The router has one 30-second deadline for the initial attempt and up to three repair retries for malformed or schema-invalid answers. A valid decision stops repairs; a valid `none` is never rerouted. With an empty `routerModel`, a Jev HTTP or connection failure triggers one reported fallback attempt with the current chat model, preserving context and validation within the remaining deadline. Explicit router selections remain pinned. Invalid inputs, invalid answers, cancellation, timeout and stale-catalog rejection do not trigger fallback. Neither repairs nor fallback can launch a duplicate workflow.
