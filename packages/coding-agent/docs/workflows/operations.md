...
The shared background widget occupies at most **10 rows**, including its final scroll hint, and at most one third of the terminal height, rounded down with a one-row minimum. Wheel or trackpad scrolling over the widget moves only its list, even at either end; scrolling outside retains normal transcript behavior. A slim scrollbar appears only when the actual allocated area overflows. There is no numeric row-range counter. Every row remains reachable when a multiline draft leaves only one widget row.
...
A nonempty exact concrete `provider/model` selection wins. Choose `openrouter/~typesafe/jev-latest` explicitly to use Jev with your existing OpenRouter login or `OPENROUTER_API_KEY`, without TypeSafe credentials. Otherwise configured direct Jev credentials, saved through `/login typesafe-ai` or supplied by `TYPESAFE_API_KEY`, select `typesafe-ai/jev-latest`; without direct Jev credentials, Atomic uses the current chat model at invocation time. `"auto"`, unknown explicit selections, and malformed settings fail rather than silently selecting another provider. This setting selects only routing inference: it does not change the chat model, workflow stage models, or the `structured_output` tool. Configured credentials do not prove service access.
