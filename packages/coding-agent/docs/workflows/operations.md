...
Model selection uses the actual stage prompt, stage name, hard constraints, available model catalog, and shipped evaluation guides. Stage system and appended system prompts are not sent to the model router. Put requirements that should influence model selection in the stage prompt; the execution session still receives its configured system prompts.

Malformed or schema-invalid routing answers receive up to three repair retries after the initial attempt, within one shared 30-second deadline. Missing guides, an empty eligible catalog, stale decisions, credential-bearing routing context, authentication/provider errors, timeout, and cancellation prevent stage execution without repair. If repairs are exhausted, no stage starts. Correct the issue and retry explicitly; Atomic never silently selects the current chat model. Choose a `routerModel` provider permitted to receive the task and supplied context. Parent chat and sibling settings are unchanged.
...
A nonempty exact concrete `provider/model` selection wins. Otherwise configured Jev credentials, saved through `/login typesafe-ai` or supplied by `TYPESAFE_API_KEY`, select `typesafe-ai/jev`; without Jev credentials, Atomic uses the current chat model at invocation time. `"auto"`, unknown explicit selections, and malformed settings fail rather than silently selecting another provider. This setting selects only routing inference: it does not change the chat model, workflow stage models, or the `structured_output` tool. Configured credentials do not prove service access.

The router has one 30-second deadline for the whole decision, including an initial attempt and up to three repair retries for malformed or schema-invalid answers. A valid decision stops repairs; a valid `none` is never rerouted. Authentication/provider failures, invalid inputs, cancellation, timeout, and stale-catalog rejection are not retried. Repairs use the same provider and context, without an agent loop, provider fallback or duplicate launch. Jev packs workflow and budget Choice questions together where context permits, preserving exact budget values rather than generating or rounding numbers. Catalogs above 255 choices use multiple tournament requests: every workflow participates, three candidates survive each batch, and finalists receive a shared comparison. `none` remains available in the final comparison even if eliminated earlier; overflow never forces a workflow launch. Large catalogs and repair attempts can increase latency and usage, and grouping can affect selection. State is never trimmed, and no partial decision can launch a workflow. See [structured decision limits](/sdk/structured-decisions#provider-behavior-and-limits) for estimated context packing and provider context errors.

### Read the decision

Route results appear as structured JSON inside a `WORKFLOW ROUTE` box. They expose `workflowId`, `routerDecision`, and the named selection's actual `inputSchema` with defaults. Read the selection from `routerDecision.workflowType` and the estimate from `routerDecision.estimatedDuration`, not top-level fields. The nested decision also includes the preserved `maxBudget` override declaration; omitted budget fields inherit their limits at execution. Routing failures have no `routerDecision`; inspect the error instead. Reservations are not executing runs and do not appear in run listings. Unused reservations expire with the owning tool/session and invalidate when the registry or selected schema changes; route again rather than reusing a stale ID. Admitted instances retain their caller/session ownership across tool recreation, reload and restart. Model-tool inspection and control authorize the resolved target, including implicit active-run targets; bulk controls reject mixed ownership before changing any run. Explicit user `/workflow` commands retain their direct-access exception.
...
Aborting a call is the point of no return: that callback's executor is already committed to suspending. If the durable paused transition then fails or is refused, Atomic still records the pause locally rather than reporting a stopped run as running. Existing usable durable progress stays resumable; the failed transition does not create progress for a run that has none. After successful database admission, a quit persistence error distinguishes the local stop from a confirmed database pause, and running `/workflow quit` again re-attempts the failed transition. During admission, quit acknowledges the local stop immediately and reports a later settlement failure through run status instead. Repeating quit does not repair an incomplete durable identity. Retain the run ID and follow the database recovery guidance below.
...
Atomic checks PostgreSQL readiness and matches the server to the managed data and process identity before attachment. A listening TCP port alone is not sufficient. If an existing cluster has no trusted ownership records, Atomic refuses to adopt or initialize it. Preserve its data and use an explicit `DBOS_SYSTEM_DATABASE_URL` for that database instead. An explicit URL uses only that endpoint, never alternate managed ports.
...
Focus a tool node and press Enter, click it, or choose it from the switcher to open its read-only operator card. The card uses a status-tinted shaded rectangle with the main-chat tool block's padding and header/body gap, inset from the orchestrator header and footer. It contains a `$ <tool-name>` header, an optional short argument summary, and the result or error. Running and completed headers have no status marker; pending, failed, cached, and cancelled headers keep quiet markers.

The card starts collapsed. It wraps the bounded result or error, then shows its last visual rows. When expansion is bound, `... (N earlier lines, ctrl+o Expand)` appears above the tail. Use `app.tools.expand`, normally `ctrl+o`, to show the full bounded result and then a muted callback-source block when source exists. Source is captured with `fn.toString()` at registration; inspection never re-executes the callback or reads a file.
...
The footer shows `Took` for settled calls or `Elapsed` for running calls, at the same second resolution as main-chat tools. Cached/replayed markers remain a quiet suffix. There is no ARGS/RESULT/SOURCE/TIMING/MARKERS debug table or raw clock display.

Payloads stay width-safe and mark truncation with `… [truncated]`. Source tabs expand and control bytes become `\xNN`. Cyclic payloads, throwing `toJSON`, and throwing property getters render `<cycle>`, `<unserializable>`, and `<unreadable>` respectively instead of crashing the view. The live snapshot uses the same cap. Durable checkpoints still retain exact output, raw-args `argsHash`, and unchanged replay behavior.
...
To check a standalone runtime directly, set `runtime` to `node_modules/@bastani/atomic-natives/postgres-runtime` inside the extracted installation, then run `"$runtime/bin/postgres" --version` and `"$runtime/bin/pg_ctl" --version`. On Windows, run the corresponding `.exe` files in PowerShell. Both must succeed. The installer also verifies the extracted file inventory, library aliases and macOS/Linux dependencies required for PostgreSQL startup and workflow database operation before selecting the installation. npm installations retain automatic first-use library-link setup; if symlink creation is unavailable, Atomic copies the required libraries instead. An invalid link manifest, conflicting alias or failed copy is reported rather than silently ignored.
...
A root awaiting task-result persistence remains quit/pause-controllable and is not diagnosed as stranded. Aborting after the write starts does not leave an unobserved checkpoint rejection or block durable finalization behind that write. Catching the rejection and returning outputs cannot overwrite the requested pause. Exact-id quit or pause of a nested task tail suspends the aggregate root, so its snapshot and durable handle do not remain raw `running`.

Resume reuses the completed task result. If only a terminal stage checkpoint exists, it reuses that complete `WorkflowTaskResult` instead, without rerunning the model. This preserves exact text, including schema-backed `maxOutput` truncation, primitive structured values such as `null`, generated worktree artifacts, warnings, session, and model metadata.

Exact `workflow status <id>` uses the live tool-control registry and reports the same stranded-root diagnostic as the listing. Same-process DBOS hydration rejects unknown checkpoint history rather than skipping it.
