...
const compactViaHook: ExtensionFactory = (pi) => {
	pi.on("session_before_compact", () => ({ compactedText: "[User]: extension summary" }));
};
...
async function compactSession(harness: Harness): Promise<void> {
	...
	for (let index = 0; index < 24; index++) {
		harness.sessionManager.appendMessage({
			role: "user",
			content: `transcript ${index} ${"x".repeat(80)}`,
			timestamp: Date.now() + index,
		});
	}
	harness.session.refreshContext();
	await harness.session.compact();
	expect(harness.session.messages.slice(0, 2).map((message) => message.role)).toEqual(["system", "custom"]);
	expect(harness.session.messages[1] && isCompactionBoundary(harness.session.messages[1])).toBe(true);
}
...
describe("context handlers and system messages", () => {
	...
	// Regression pi#9789, pi#9822: pruning from the compaction boundary dropped the prompt and tool checkpoint.
	it("keeps the prompt and tools when a handler slices from the compaction boundary", async () => {
			...
			extensionFactories: [
				...
				(pi) => {
					pi.on("context", async (event) => {
						...
						const summary = event.messages.findIndex(isCompactionBoundary);
...
describe("context_with_system handlers", () => {
	...
	it("runs after context handlers on the restored transcript and sends its output verbatim", async () => {
		...
		expect(input?.[1] && isCompactionBoundary(input[1])).toBe(true);
