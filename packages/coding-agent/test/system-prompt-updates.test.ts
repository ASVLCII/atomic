...
describe("system prompt updates", () => {
	test("event.systemPrompt immediately reflects structured edits inside the same handler", async () => {
		let observed = "";
		const harness = await createHarness({
			extensionFactories: [
				(pi) => {
					pi.on("before_agent_start", (event) => {
						event.systemPromptOptions.sections.raw = "  raw\nraw  ";
						observed = event.systemPrompt;
					});
				},
			],
		});
