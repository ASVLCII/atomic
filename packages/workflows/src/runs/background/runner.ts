...
export function buildDetachedAccepted(name: string, runId: string): DetachedAccepted {
	return {
		action: "run",
		name,
		runId,
 ...
 */
export function runDetached<TInputs extends WorkflowInputValues, TRunInputs extends WorkflowInputValues = TInputs>(
