...
const MISSING_API_KEY_CODES = new Set(["missing_api_key", "api_key_missing", "no_api_key"]);

const RATE_LIMIT_CODES = new Set(["429", "rate_limit", "rate_limited", "rate_limit_exceeded", "too_many_requests"]);
...
const CANCELLED_CODES = new Set(["aborterror", "aborted", "cancelled", "canceled"]);
...
const UNKNOWN_MODEL_CODES = new Set(["unknown_model", "model_not_found", "model_not_available", "unsupported_model"]);
...
const PROVIDER_AUTH_FALLBACK_PHRASES: readonly TokenMatch[] = [["unauthorized"], ["authentication", "required"]];
...
const HTTP_RATE_LIMIT_PHRASES: readonly TokenMatch[] = [["429"], ["too", "many", "requests"]];
...
const CANCELLED_PHRASES: readonly TokenMatch[] = [["aborted"], ["cancelled"], ["canceled"]];
...
const MODEL_PROVIDER_CONTEXT = new Set(["unavailable", "overloaded", "temporarily", "service"]);

const PROVIDER_CONTEXT = new Set(["error", "failure", "failed", "overloaded", "unavailable", "temporarily", "service"]);
...
export function strongDecisionFromNormalizedCode(
...
export function weakLoginDecisionFromNormalizedCode(
	normalized: string | undefined,
): WorkflowFailureDecision | undefined {
	return normalized !== undefined && LOGIN_REQUIRED_CODES.has(normalized) ? authDecision("login_required") : undefined;
}
...
export function hasInvalidApiKeyMessage(tokens: readonly string[]): boolean {
	return (
		hasAnyPhrase(tokens, INVALID_API_KEY_PHRASES) ||
		(hasPhrase(tokens, ["api", "key"]) && tokenNearAny(tokens, "key", INVALID_API_KEY_CONTEXT, 6))
	);
}

export function decisionFromMessageTokens(
...
export function decisionFromStatus(
...
export function classifyFallbackProviderAuthMessage(
	message: string,
	tokens: readonly string[],
): WorkflowFailureDecision | undefined {
...
export function canUseLoginClassificationBeforeWrapper401(
	...
	return (
		classification.evidence === "weak_signal" ||
...
export function classificationFromNormalizedCode(
	...
	return weak !== undefined ? { weak: classificationForDecision(weak, source, message, "weak_signal") } : {};
}

export function fallbackDecisionFromMessage(
	message: string,
	name: string | undefined,
): WorkflowFailureDecision | undefined {
