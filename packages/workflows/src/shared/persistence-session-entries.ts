...
export function appendStageEnd(api: PersistenceAPI, payload: StageEndPayload, opts?: { emitMessage?: boolean }): void {
	if (typeof api.appendEntry !== "function") return;
	api.appendEntry("workflow.stage.end", {
		...
		...(payload.replayedFromStageId !== undefined ? { replayedFromStageId: payload.replayedFromStageId } : {}),
		...(payload.replayed !== undefined ? { replayed: payload.replayed } : {}),
		...(payload.status === "completed" && payload.workflowChild !== undefined
			? { workflowChild: payload.workflowChild }
			: {}),
	});
...
export function appendRunEnd(api: PersistenceAPI, payload: RunEndPayload): void {
	...
	api.appendEntry("workflow.run.end", {
		...
		...(terminalPayload.failureRecoverability !== undefined
			? { failureRecoverability: terminalPayload.failureRecoverability }
			: {}),
		...(terminalPayload.failureDisposition !== undefined
			? { failureDisposition: terminalPayload.failureDisposition }
			: {}),
