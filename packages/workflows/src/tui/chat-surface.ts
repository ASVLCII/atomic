...
export function renderRoundedBoxLines(opts: RenderRoundedBoxOpts & { width: number }): string[] {
	...
	const titleText =
		opts.title && opts.title.length > 0 ? ` ${truncateToWidth(opts.title, titleBudget, ELLIPSIS)} ` : "";
...
function renderFlatBandThemed(
	...
	const badgeW =
		badges.length > 0 ? badges.reduce((a, b) => a + visibleWidth(b.text), 0) + (badges.length - 1) * 2 : 0;

	const leftPad = 1;
	const rightPad = 1;
	const filler = Math.max(1, width - leftPad - labelW - subtitleW - badgeW - rightPad);
...
function renderFlatBandPlain(label: string, subtitle: string, badges: readonly FlatBandBadge[], width: number): string {
	const labelText = `[ ${label} ]`;
	const labelW = visibleWidth(labelText);
...
function renderTaggedCardThemed(opts: RenderTaggedCardOpts, theme: GraphTheme, width: number): string {
	...
	const tagSubtitleSeg = opts.tagSubtitle ? `  ${muted}${opts.tagSubtitle}${RESET}` : "";
	...
	const titleSeg = titleVisible ? `  ${text}${BOLD}${titleVisible}${RESET}` : "";
	...
	const gap = Math.max(1, width - row1StripePrefixW - tagW - titleSegW - suffixSegW - tagSubtitleW - trailingW - 1);
	const row1 =
...
function renderTaggedCardPlain(opts: RenderTaggedCardOpts, width: number): string {
	...
	const suffixSeg = opts.titleSuffix ? `  ${opts.titleSuffix}` : "";
	const suffixSegW = opts.titleSuffix ? titleSuffixGap + titleSuffixW : 0;
	const gap = Math.max(1, width - row1StripePrefixW - tagW - titleW - suffixSegW - tagSubtitleW - trailingW - 1);
...
export function progressStrip(cells: readonly ProgressCell[], budget: number, theme?: GraphTheme): string {
...
function stageColor(status: StageStatus, theme: GraphTheme): string {
	switch (status) {
		case "completed":
			return hexToAnsi(theme.success);
		case "running":
			return hexToAnsi(theme.warning);
		case "paused":
			return hexToAnsi(theme.warning);
		case "failed":
			return hexToAnsi(theme.error);
		case "awaiting_input":
...
export function renderHintRows(rows: readonly HintRow[], theme?: GraphTheme): string {
	...
	const indent = " ";
	if (!theme) {
		return rows.map((r) => `${indent}▸ ${r.command}  ${r.hint}`).join("\n");
	}
	const dim = hexToAnsi(theme.dim);
	const accent = hexToAnsi(theme.accent);
	return rows.map((r) => `${indent}${dim}▸${RESET} ${accent}${r.command}${RESET}  ${dim}${r.hint}${RESET}`).join("\n");
}
