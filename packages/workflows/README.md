...
When the user delegates the best-stage-model choice, use `model: "auto"` on stages or shared chain/parallel options. Routing selects from the actual prompt after input interpolation, supplied context, available model/effort pairs, and shipped evaluation guidance. Preserve explicit model, effort, and `modelConstraints` requests. Automatic model/effort assignments remain pending until routing; the following manual assignment guidance applies to deliberately concrete stages.
...
For example, submit factual task context and known unbound inputs without choosing a workflow:

```json
{
  "action": "run",
  "inputs": { "prompt": "Map session persistence by subsystem and synthesize cited findings", "max_branches": 3 },
  "state": {
    "literalRequest": "Map session persistence by subsystem and synthesize cited findings. Do not change code.",
    "intent": "Understand the end-to-end persistence flow.",
    "conversation": [{ "role": "user", "text": "Map session persistence by subsystem and synthesize cited findings. Do not change code." }],
    "constraints": ["Read-only research; do not change code."],
    "executionPreference": "unspecified",
    "documents": [{ "source": "research acceptance criteria", "content": "Cite concrete repository paths for each subsystem and reconcile conflicting findings in the synthesis." }]
  }
}
```

Read `routerDecision` and `estimatedDuration` before reporting any selection or estimate. The router alone chooses a registered workflow or `none` from neutral user context, including explicit named or inline preferences and whether workflow overhead makes sense. `none` means conversation, clarification or inline work, not completion or refusal. A selected workflow launches after supplied inputs and defaults validate; `needs_input` returns the exact input contract without admission. Supply actual missing values and retry explicitly; each retry routes anew, without stale-input remapping. Duration values are `unknown`, `under_5_minutes`, `5_to_15_minutes`, `15_to_60_minutes`, `1_to_4_hours`, or `over_4_hours`, unmeasured task/catalog estimates rather than guarantees or budgets. Only add `state.userBudget` for an explicit user instruction; top-level `budget` must match its exact limits. Legacy run `workflow` names are ignored and do not establish user intent.
