...
For example, after inspecting the workflow's input contract, a model-tool launch supplies actual task context as well as inputs:
...
```json
{
  ...
  "state": {
    "literalRequest": "Use a workflow to map session persistence by subsystem and synthesize cited findings. Do not change code.",
    "intent": "Partition repository research and synthesize the end-to-end persistence flow.",
    "conversation": [{ "role": "user", "text": "Use a workflow to map session persistence by subsystem and synthesize cited findings. Do not change code." }],
...
Read `routerDecision` before continuing: a matching workflow may launch; `none` means continue inline, not task completion; another selection launches nothing and requires freshly prepared inputs/state. Unknown workflows fail before inference and allocate no run. This example inherits budget limits; only add `state.userBudget` for an explicit user instruction, and make any top-level `budget` match those limits exactly.
