	...
	while (Date.now() < deadline) {
		for (const runSnapshot of store.runs()) {
			const stage = runSnapshot.stages.find(
				(candidate) => candidate.status === "awaiting_input" && candidate.promptFootprint?.kind === "custom",
			);
...
function resolveExecutorCustomPrompt<T>(runId: string, stageId: string, value: T): void {
