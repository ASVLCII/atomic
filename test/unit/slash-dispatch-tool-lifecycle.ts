...
describe("tool run-control actions", () => {
	...
	test.sequential("registered workflow tool content defaults to path-only transcripts and supports explicit previews", async () => {
		...
		const referenceDetails = textResult.details as Extract<WorkflowToolResult, { action: "transcript" }>;
		...
		assert.match(
			referenceDetails.lazyReadPrompt ?? "",
			/Read it lazily from C:\\Users\\atomic runner\\tool-content\.jsonl/,
		);
		...
		const explicitTextContent = explicitTextBlock.type === "text" ? explicitTextBlock.text : "";
		assert.ok(explicitTextContent.includes(longText), "explicit tail should inline the requested transcript entry");
		const explicitTailDetails = explicitTextResult.details as Extract<WorkflowToolResult, { action: "transcript" }>;
		...
		const explicitLimitContent = explicitLimitBlock.type === "text" ? explicitLimitBlock.text : "";
		assert.ok(explicitLimitContent.includes(toolOutput));
		assert.ok(explicitLimitContent.includes(longText));
		const explicitLimitDetails = explicitLimitResult.details as Extract<WorkflowToolResult, { action: "transcript" }>;
		...
		const defaultJsonText = defaultJsonBlock.type === "text" ? defaultJsonBlock.text : "{}";
