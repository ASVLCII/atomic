...
import { _runAgentPrompt, prompt, sendUserMessage } from "../../packages/coding-agent/src/core/agent-session-prompt.js";
import type {
...
test("production prompt wiring holds idle admission until the first agent turn starts", async () => {
	...
	const surface = {
		agent: {
			...
			prompt(messages: AgentMessage | AgentMessage[]) {
				...
				return firstTurn.promise.finally(() => {
					streaming = false;
				});
			},
		},
	...
	firstTurn.resolve();
	const [firstOutcome, secondOutcome] = await Promise.all([Promise.allSettled([first]), Promise.allSettled([second])]);
