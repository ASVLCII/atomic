  ...
  run: async () => ({}),
		...
		try {
			assert.equal(
				globalThis[marker],
				0,
				"session_start should not evaluate package workflow modules synchronously",
			);
	...
	test.sequential("/workflow list proceeds when no UI is available and emits printable content", async () => {
		...
		assert.match(listMessage.content ?? "", /fan-out-and-synthesize|tournament|open-claude-design/);
	...
	test.sequential("/workflow inputs <known> emits displayable command output in headless mode", async () => {
		...
		assert.match(output.content ?? "", /INPUTS FOR TOURNAMENT|Inputs for "tournament":/);
	...
	test.sequential("/workflow <known> --help emits displayable command output and skips dispatch headlessly", async () => {
		...
		assert.equal(output.display, true);
		assert.equal(typeof output.content, "string");
		assert.match(output.content ?? "", /INPUTS FOR FAN-OUT-AND-SYNTHESIZE|Inputs for "fan-out-and-synthesize":/);
		assert.deepEqual(output.details, {
			command: "help",
