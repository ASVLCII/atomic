...
class HydrationCapableBackend extends InMemoryDurableBackend {
	hydrateCalls = 0;
	async hydrateWorkflow(): Promise<void> {
		this.hydrateCalls += 1;
	}
}
...
class UnclassifiedHydrationBackend extends HydrationCapableBackend {
	loadable = false;
	override isWorkflowLoadable(): boolean {
...
describe("workflow lazy-startup review follow-up fixes", () => {
	...
	test("/workflow list surfaces lazy discovery failures as warnings", async () => {
		...
		await workflowCmd.options.handler?.("list", {
			hasUI: true,
			ui: {
				notify: (message) => {
				...
				dispatch: async (): Promise<WorkflowToolResult> =>
					canSeeLazyWorkflow
						...
						: {
								...
								runId: "",
								status: "failed",
								error: "Workflow not found: lazy model run",
								stages: [],
							},
			} as unknown as ExtensionRuntime;
		};
