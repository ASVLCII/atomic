...
describe("classifyWorkflowFailure", () => {
	...
	test("does not treat log information/input errors as auth failures", () => {
		for (const message of ["failed to log information about request", "failed to log input before validation"]) {
	...
	test("does not treat generic domain/tool model errors as provider outages", () => {
		for (const message of ["domain model validation failed", "invalid model parameter passed to tool"]) {
			const failure = classifyWorkflowFailure(new Error(message));
			assert.equal(failure.kind, "unknown");
			assert.equal(failure.retryable, false);
		}
	...
	test("classifies git subprocess timeouts without treating them as repository setup errors", () => {
		const failure = classifyWorkflowFailure(
			new Error(
				"Timed out while checking the Git repository for gitWorktreeDir from /repo. Git reported: git command timed out after 60000ms (ETIMEDOUT): spawnSync git ETIMEDOUT",
			),
		);
