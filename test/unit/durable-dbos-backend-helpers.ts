...
export function createMockSdk(): DbosSdkHandle & { state: MockDbosState } {
	...
	return {
		...
		async cancelWorkflow(workflowId) {
			state.cancels.push(workflowId);
		},
		async resumeWorkflow(workflowId) {
			state.resumes.push(workflowId);
		},
		...
		async deleteWorkflowData(workflowId) {
			...
			for (const key of state.steps.keys()) if (key.startsWith(`${workflowId}:checkpoint:`)) state.steps.delete(key);
		},
	};
}

export function seedMockWorkflow(
	sdk: ReturnType<typeof createMockSdk>,
	info: Partial<DbosWorkflowInfo> & { workflowId: string },
): void {
	...
	const metadata = {
		...
		pendingPrompts: 0,
		createdAt: timestamp,
		promptReservationEpoch: "seed-epoch",
		updatedAt: timestamp,
	};
	sdk.state.steps.set(`${info.workflowId}:checkpoint:__atomic_metadata:${timestamp}:seed`, {
...
export function seedMockCheckpoint(
	sdk: ReturnType<typeof createMockSdk>,
	workflowId: string,
	cp: DurableCheckpoint,
): void {
	const current =
		cp.kind === "stage" && cp.topology === undefined
