...
describe("tool run-control actions", () => {
	...
	test("active blocked continuation atomically claims its durable source", async () => {
		...
		const def = workflow({
			...
			description: "",
			inputs: {},
			outputs: { value: Type.Optional(Type.Any()) },
		...
		const results = await Promise.all([runtime.resumeFailedRun(sourceRunId), runtime.resumeFailedRun(sourceRunId)]);
